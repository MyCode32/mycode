...
# ================================================================
APServiceConfigDirectory=$mycodeWorkspacePath
# ================================================================
...
function ap_service_stop() {
  if [ "$APServicePID" ]; then
    kill $APServicePID &> $mycodeOutputDevice
  fi
...
function ap_service_reset() {
  ...
  macchanger -p $APServiceInterface &> $mycodeOutputDevice
  sleep 0.25
...
function ap_service_route() {
  echo "APService: No custom routes for hostapd" > $mycodeOutputDevice
}
...
channel=$APServiceChannel" \
  ...
  macchanger --mac=$APServiceMAC $APServiceInterface &> $mycodeOutputDevice
  sleep 0.5
...
function ap_service_start() {
  ...
  xterm $mycodeHoldXterm $TOP -bg "#000000" -fg "#FFFFFF" \
    -title "mycode AP Service [hostapd]" -e \
